...
st.set_page_config(
    page_title="Mining Process Optimization Dashboard", 
    layout="wide",
...
def render_header():
    logo_path = "C:/Users/a1900269/Downloads/genesis-mining-logo-vector.png"
...
st.sidebar.subheader("Process Control")
Blasting_target_p80 = st.sidebar.slider(
    "Blasting Target P80 (mm)", 500.0, 700.0, 600.0, step=20.0, key="blasting_target_p80"
)
Screening_target_p80 = st.sidebar.slider(
    "Screening Target P80 (mm)", 0.10, 2.0, 1.5, step=0.5, key="screening_target_p80"
)
Crushing_target_p80 = st.sidebar.slider(
    "Crushing Target P80 (mm)", 50.0, 100.0, 80.0, step=10.0, key="crusher_target_p80"
)
st.sidebar.subheader("Economic Parameters")
cost_per_ton = st.sidebar.number_input(
    "Processing Cost ($/ton)", 1.0, 10.0, 2.5, step=0.1, key="cost_per_ton"
)
energy_cost = st.sidebar.number_input(
    "Energy Cost ($/kWh)", 0.05, 0.5, 0.12, step=0.01, key="energy_cost"
)
if st.sidebar.button("Optimize Process", key="optimize_button"):
    st.sidebar.success("Process optimized for target P80!")

# ------------------------------
# Custom CSS for "box" containers
# ------------------------------
box_style = """
<style>
.box {
    background-color: #2D3B44; /* Dark background */
    padding: 1rem;
    border-radius: 8px;
    margin-bottom: 1rem;
    color: #FFFFFF;
...
def load_models(base_path="models"):
    """
    Load all models from a given directory structure using relative paths.
    Models are expected to be located in subdirectories according to process stage.
    """
    models = {}
    base_dir = Path(base_path)
    
    def safe_load(model_relative_path):
    ...
    # Load blasting models
    models["blasting_p20"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/blasting/best_model_p20.pkl")
    models["blasting_p50"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/blasting/best_model_p50.pkl")
    models["blasting_p80"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/blasting/best_model_p80.pkl")
    
    # Load screening models
    models["OnScreen_p20"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_OS_p20.pkl")
    models["OnScreen_p50"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_OS_p50.pkl")
    models["OnScreen_p80"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_OS_p80.pkl")
    models["OnScreen_Massflow"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_OS_Mass.pkl")
    
    models["UnderScreen_p20"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_US_P20.pkl")
    models["UnderScreen_p50"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_US_P50.pkl")
    models["UnderScreen_p80"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_US_P80.pkl")
    models["UnderScreen_Massflow"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_US_Mass.pkl")
    
    # Load Crushing models
    models["Crusher_p20"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_Crusher_p20.pkl")
    models["Crusher_p50"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_Crusher_p50.pkl")
    models["Crusher_p80"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_Crusher_p80.pkl")
    models["Crusher_Mass"] = joblib.load("C:/P.Nobahar/Thesis/Meta models/3M scenarios/Results/Screen - Crusher/best_model_Crusher_Mass.pkl")
    
    return models

models = load_models()  # Load models at startup
...
def create_sankey_diagram(total_mass, os_mass, us_mass):
    fig = go.Figure(data=[go.Sankey(
        domain=dict(
            x=[0.0, 1.0],  # Use full width for the diagram
            y=[0.0, 1.0]
        ),
        node=dict(
            pad=7.5,
    ...
    fig.update_layout(
        ...
        font_size=15,
        width=1200,   # Increase overall width to stretch the diagram horizontally
        height=400,
...
# Inject some custom CSS for your "box" containers
box_style = """
<style>
.box {
    background-color: #2D3B44; /* Dark background color */
    padding: 1rem;            /* Inner spacing */
    border-radius: 8px;       /* Rounded corners */
    margin-bottom: 1rem;      /* Spacing below each box */
...
st.markdown("""
    <style>
    .my-box {
        max-width: 200px; /* Decreases the maximum width */
        margin: auto;     /* Centers the box (optional) */
        padding: 10px;    /* Reduces the padding */
    }
    </style>
...
if current_page == "👁️ Overview":
    ...
    with chart2:
        # Sample data for energy consumption (kWh) per mining section
        energy_data = {
            "Blasting": 250,
            "Screening": 150,
            "Crushing": 200,
    ...
    with chart3:    
        # Sample data for cost share ($/ton) per mining section
        cost_data = {
            "Blasting": 2.5,
            "Screening": 1.8,
            "Crushing": 4.0,
       ...
       # Coordinates for your project location (example: Adelaide, Australia)
 
    with chart4:
        latitude, longitude = -34.9285, 138.6007

        # Create a folium map centered on the project location
        m = folium.Map(location=[latitude, longitude], zoom_start=3)

        # Add a marker to indicate the exact project location
        folium.Marker(
            [latitude, longitude],
...
- Adjust eccentric speed to reduce power consumption""")
    ...
    feed_mass = (
        st.session_state.process_data['screening']['onscreen']['mass'] + 
        st.session_state.process_data['screening']['underscreen']['mass']
    )
    ...
    with blast_col1:
        ...
        with st.container():
            ...
            # Calculate the powder factor using the input values.
    ...
    with blast_col2:
        ...
        # Size distribution curve calculations
        ...
        # Define sensitivity ranges for parameters
    sensitivity_ranges = {
    ...
    # User selects which parameter to vary
    ...
    # Base input for the prediction (ensure these variables are defined earlier or via user input)
    ...
    # Create three columns: one each for P20, P50, and P80
    col1, col2, col3 = st.columns(3)
    metrics = [("P20", col1), ("P50", col2), ("P80", col3)]

    for metric, col in metrics:
        with col:
            st.subheader(f"Sensitivity Analysis for {metric}")
            # Retrieve the corresponding model (e.g., "blasting_p20" for P20)
            ...
            for val in sensitivity_ranges[sensitivity_param]:
                ...
                # Compute SHAP values for the modified input
                shap_values_metric = explainer(modified_input)
                # Get prediction for the current metric using the chosen model
                prediction_metric = chosen_model.predict(modified_input)[0]
                # Determine the index of the parameter to vary in the base input
                param_index = list(base_input.columns).index(sensitivity_param)
                # Extract the SHAP impact for that parameter
    ...
    with col1:
        ...
        blasting_area = st.number_input("Blasting area (m2)", min_value=500, value=2000, step=50)
    ...
    # Automatically calculate charges
    ...
    with screen_col2:
        ...
        with results_tab2:
            ...
            f"""
            <div style='padding:10px;border-radius:8px;border:1px solid #444;background-color:#3b3b3b;color:#FFFFFF;'>
                <strong>Screen Efficiency: { ((Underscreen_oversize_value-blasting_oversize_value)/(Underscreen_oversize_value-Onscreen_oversize_value))*100:.1f} % | 
            </div>
            """,
            unsafe_allow_html=True
              )   
            ...
            for css in css_values:
                onscreen_value = np.interp(css, sizes_os, percentages_os)
                underscreen_value = np.interp(css, sizes_us, percentages_us)
                # Example calculation for efficiency, assuming a simple ratio
    ...
    for feed_val in feed_values:
        ...
        # Call the function and extract the numeric value from the tuple.
        result = crushing_prediction(css, onscreen_p20, onscreen_p50, feed_val, onscreen_mass)
        
        # Assuming the first element is the desired crusher P80 value:
        p80_val = result[2] if isinstance(result, tuple) else result

        # Calculate crusher power using the extracted numeric p80_val
    ...
    # Create the DataFrame with explicit column names matching what we want to use.
    feed_df = pd.DataFrame({
        'Feed (mm)': feed_values,
        'Crusher P80 (mm)': p80_values,  # renamed column to "Crusher P80 (mm)"
    ...
    # Trace for Crusher P80 (left y-axis)
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title='Feed (mm)',
            range=[feed_values.min(), feed_values.max()]  # sets x-axis range from 500 to 600
        ),
        yaxis=dict(
            title='Crusher P80 (mm)',
            side='left',
            showgrid=False,
            range=[50, 150]  # y1 range from 50 to 150
        ),
        yaxis2=dict(
            title='Power (kW)',
            side='right',
